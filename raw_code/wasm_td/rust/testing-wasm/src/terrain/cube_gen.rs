...
impl CubeGenerator {
    ...
    pub fn generate_mesh(&self, dim: &mut Vec3, color: Vec3) -> Float32Array {
        ...
        for i in 1..(points - 1) {
            let angle1 = 0.0;
            let angle2 = 2.0 * (i as f32) * PI / (points as f32);
            let angle3 = 2.0 * ((i + 1) as f32) * PI / (points as f32);
        ...
        for i in 0..(heights.len() - 1) {
            for j in 0..points {
                let angle1 = 2.0 * (j as f32) * PI / (points as f32);
                let angle2 = 2.0 * ((j + 1) as f32) * PI / (points as f32);
